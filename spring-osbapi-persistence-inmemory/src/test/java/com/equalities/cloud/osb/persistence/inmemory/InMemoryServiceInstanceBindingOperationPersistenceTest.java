...
class InMemoryServiceInstanceBindingOperationPersistenceTest {

  static final Duration duration = Duration.ofSeconds(3);

  private Map<String, ServiceInstanceBindingOperationEntity> memory;
  private InMemoryServiceInstanceBindingOperationPersistence instance;
  ...
  void setUp() throws Exception {
    memory = new HashMap<>();
    instance = new InMemoryServiceInstanceBindingOperationPersistence(memory);
  }

  @Test
  void testInMemoryServiceInstanceBindingOperationPersistence() {
    new InMemoryServiceInstanceBindingOperationPersistence();
  }

  @Test
  void testInMemoryServiceInstanceBindingOperationPersistenceDuration() {
    new InMemoryServiceInstanceBindingOperationPersistence(Duration.ofMillis(20));
  }

  @Test
  void testInMemoryServiceInstanceBindingOperationPersistenceMapOfStringServiceInstanceBindingOperationEntity() {
    new InMemoryServiceInstanceBindingOperationPersistence(memory);
  }
  ...
  void testReadByOperationId() {
    ...
    instance.insert(entity).block();
    assertThat(memory).containsEntry(entity.getId(), entity);
    ServiceInstanceBindingOperationEntity readEntity = instance.readByOperationId(id).block();
