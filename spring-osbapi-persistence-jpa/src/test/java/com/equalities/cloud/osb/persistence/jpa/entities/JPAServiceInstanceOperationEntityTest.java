...
class JPAServiceInstanceOperationEntityTest {
  ...
  @Mock
  ServiceInstanceOperationEntity osbEntity;
  
  @Mock
  JPAServiceInstanceOperationEntity jpaEntity;
  ...
  void testJpaType() {
    ...
    jpaEntity = JPAServiceInstanceOperationEntity.jpaType(osbEntity);
  ...
  @Test
  void testOsbTypeJPAServiceInstanceOperationEntity() {
    ...
    osbEntity = JPAServiceInstanceOperationEntity.osbType(jpaEntity);
  ...
  @Test
  void testOsbTypeJPAServiceInstanceOperationEntityReturnsNullOnNullInut() {
    assertThat(JPAServiceInstanceOperationEntity.osbType((JPAServiceInstanceOperationEntity)null)).isNull();
  }

  @Test
  void testOsbTypeOptionalOfJPAServiceInstanceOperationEntity() {
    when(jpaEntity.getId()).thenReturn(ID);
    when(jpaEntity.getServiceInstanceId()).thenReturn(SERVICE_INSTANCE_ID);
    when(jpaEntity.getServiceDefinitionId()).thenReturn(SERVICE_DEFINITION_ID);
    when(jpaEntity.getServicePlanId()).thenReturn(SERVICE_PLAN_ID);
    ...
    osbEntity = JPAServiceInstanceOperationEntity.osbType(Optional.of(jpaEntity));
  ...
  @Test
  void testOsbTypeOptionalOfJPAServiceInstanceOperationEntityReturnsNullOnEmptyInput() {
    assertThat(JPAServiceInstanceOperationEntity.osbType((Optional.empty()))).isNull();
  }
}
